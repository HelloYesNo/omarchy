...
if [ -n "$(lspci | grep -i 'nvidia')" ]; then
  ...
  echo "options nvidia_drm modeset=1" | tee /etc/modprobe.d/nvidia.conf >/dev/null
  ...
  sed -i -E 's/ nvidia_drm//g; s/ nvidia_uvm//g; s/ nvidia_modeset//g; s/ nvidia//g;' "$MKINITCPIO_CONF"
  # Add the new modules at the start of the MODULES array
  sed -i -E "s/^(MODULES=\\()/\\1${NVIDIA_MODULES} /" "$MKINITCPIO_CONF"
  # Clean up potential double spaces
  sed -i -E 's/  +/ /g' "$MKINITCPIO_CONF"
