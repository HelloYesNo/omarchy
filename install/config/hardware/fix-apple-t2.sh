...
if lspci -nn | grep -q "106b:180[12]"; then
  ...
  echo "apple-bce" | tee /etc/modules-load.d/t2.conf >/dev/null

  echo "MODULES+=(apple-bce usbhid hid_apple hid_generic xhci_pci xhci_hcd)" | tee /etc/mkinitcpio.conf.d/apple-t2.conf >/dev/null

  cat <<EOF | tee /etc/modprobe.d/brcmfmac.conf >/dev/null
# Fix for T2 MacBook WiFi connectivity issues
options brcmfmac feature_disable=0x82000
EOF

  mkdir -p /etc/limine-entry-tool.d
  cat <<EOF | tee /etc/limine-entry-tool.d/t2-mac.conf >/dev/null
